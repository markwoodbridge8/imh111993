...
import {DaffiWalletTransaction, SignerTransaction} from "../model/daffiWalletModels";
...
function composeTransaction(transaction: SignerTransaction, signerAddress?: string) {
  let signers: DaffiWalletTransaction["signers"];

  if (signerAddress && !(transaction.signers || []).includes(signerAddress)) {
    signers = [];
  }

  const txnRequestParams: DaffiWalletTransaction = {
